...
import { Users, AlertTriangle, IndianRupee, MessageCircle, TrendingUp, Dumbbell, User, Activity } from "lucide-react";
    ...
    <div className="flex items-end gap-[3px] h-10">
      {data.map((val, i) => (
        <div
          ...
          style={{ height: `${(val / max) * 100}%`, background: color, opacity: 0.4 + (i / data.length) * 0.6 }}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="relative w-28 h-28">
      <svg className="w-full h-full -rotate-90" viewBox="0 0 80 80">
        ...
        <circle cx="40" cy="40" r="36" fill="none" stroke="#e63946" strokeWidth="6" strokeDasharray={`${gymStroke} ${circumference}`} strokeLinecap="round" />
        <circle cx="40" cy="40" r="36" fill="none" stroke="#22c55e" strokeWidth="6" strokeDasharray={`${circumference - gymStroke} ${circumference}`} strokeDashoffset={`-${gymStroke}`} strokeLinecap="round" />
      </svg>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Welcome back, <span className="text-[#e63946]">Anil sir</span> 👋</h1>
          <p className="text-gray-500 mt-1 text-sm">{new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })}</p>
        </div>
        <a href="/admin/members/register" className="hidden md:flex items-center gap-2 bg-[#e63946] text-white px-5 py-2.5 rounded-xl font-medium transition-all hover:brightness-110 active:scale-95">+ Register</a>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-[#e63946]/10 via-[#0a0a0a] to-[#0a0a0a] border border-[#e63946]/20 rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2"><IndianRupee className="w-4 h-4 text-[#e63946]" /><span className="text-gray-400 text-sm">Total Due</span></div>
          ...
          <div className="absolute bottom-4 right-4"><Activity className="w-16 h-16 text-[#e63946]/10" /></div>
        </div>
        <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 flex items-center gap-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#e63946]" /><span className="text-sm text-gray-300">Gym — {gymCount}</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-green-500" /><span className="text-sm text-gray-300">PT — {ptCount}</span></div>
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden">
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-lg font-bold flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-400" />Pending Fees</h2>
          <span className="text-[10px] bg-red-500/10 text-red-400 px-3 py-1 rounded-full font-bold">{dueMembers.length} OVERDUE</span>
        </div>
        {dueMembers.length === 0 ? (
          <div className="p-16 text-center">
            <div className="w-16 h-16 mx-auto rounded-full bg-green-500/10 flex items-center justify-center mb-4"><TrendingUp className="w-8 h-8 text-green-500/50" /></div>
            ...
            {dueMembers.map((member) => (
              <div key={member.id} className="p-4 flex items-center gap-3 hover:bg-white/[0.02] transition-colors">
                {member.photo_url ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#e63946]/10 flex items-center justify-center text-[#e63946] font-bold text-sm">{member.name.charAt(0)}</div>
                )}
                ...
                <a href={generateWhatsAppLink(member.phone, member.name, member.fee_amount)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:brightness-110 active:scale-95 transition-all">
      ...
      <a href="/admin/members/register" className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-[#e63946] rounded-full flex items-center justify-center shadow-lg shadow-[#e63946]/30 active:scale-90 transition-transform z-40">
